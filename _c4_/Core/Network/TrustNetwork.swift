...
enum _c4_NetworkProtocolError: LocalizedError {
    case missingPrices
    case missingContractInfo
}

protocol NetworkProtocol: _c4_NetworkProtocol {
...
class _c4_Network: NetworkProtocol {

    static let deleteMissingInternalSeconds: Double = 60.0
    static let deleyedTransactionInternalSeconds: Double = 60.0
    let provider: MoyaProvider<_c4_Service>
    let config: Config
    let balanceService: TokensBalanceService
    let account: Wallet

    required init(provider: MoyaProvider<_c4_Service>, balanceService: TokensBalanceService, account: Wallet, config: Config) {
    ...
    func tickers(with tokenPrices: [TokenPrice]) -> Promise<[CoinTicker]> {
        return Promise { seal in
            ...
            provider.request(.prices(tokensPriceToFetch)) { result in
                guard case .success(let response) = result else {
                    seal.reject(_c4_NetworkProtocolError.missingPrices)
                    return
                }
                do {
                    let rawTickers = try response.map([CoinTicker].self, atKeyPath: "response", using: JSONDecoder())
        ...
        Session.send(EtherServiceRequest(batch: BatchFactory().create(request))) { [weak self] result in
            ...
            case .failure(let error):
                ...
                case .responseError(let error):
                    ...
                    switch error {
                    case .responseError:
                        if transaction.date > Date().addingTimeInterval(_c4_Network.deleteMissingInternalSeconds) {
                ...
                case .success(let response):
                    do {
                        ...
                        guard let token = tokens.first(where: { $0.address.eip55String == Address(string: token)?.eip55String }) else {
                             return seal.reject(_c4_NetworkProtocolError.missingContractInfo)
                        }
                        seal.fulfill(token)
                    } catch {
                        seal.reject(error)
