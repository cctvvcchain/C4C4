...
class TokensViewModelTest: XCTestCase {
    let model = TokensViewModel(address: .make(), store: FakeTokensDataStore(), tokensNetwork: FakeTokensNetwork(provider: _c4_ProviderFactory.makeProvider(), balanceService: FakeGetBalanceCoordinator(), account: .make(), config: .make()))
